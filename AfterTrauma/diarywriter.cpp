...
void DiaryWriter::write( PagedPNG* writer ) {
    //
    //
    //
    m_adjustedPadding = _inchesToPixels( m_padding, writer );
    ...
    /*
#ifdef Q_OS_IOS
    m_painter.begin( writer->imageptr() );
#else
    m_painter.begin( writer );
#endif
    m_painter.setViewport(writer->paintRect());
    QRect r = m_painter.viewport();
    r.moveTo(0,0);
    */
    m_painter.begin( writer->imageptr() );
...
void DiaryWriter::save( const QVariantList& entries, const QString& filePath ) {
    ...
    */
    ...
    if ( documentId.length() > 0 ) {
        m_entries = entries;
        qDebug() << "DiaryWriter::save : writing";
        PNGGenerator::shared()->write(documentId,this);
        ...
        qDebug() << "DiaryWriter::save : removing temporary file";
        PNGGenerator::shared()->removeDocument(documentId);
        m_entries.clear();
        qDebug() << "DiaryWriter::save : done";
        emit saved( filePath );
    }
...
void DiaryWriter::_paintEntry( const QVariantMap& entry, PagedPNG* writer, const QRect& r, QPoint& cp ) {
    //
    // entry header
    //
    _paintEntryHeader(entry, writer, r, cp );
...
void DiaryWriter::_paintEntryHeader( const QVariantMap& entry, PagedPNG* writer, const QRect& r, QPoint& cp ) {
    //
    // check for space
    //
    int headerHeight = _inchesToPixels(3.5,writer);
...
void DiaryWriter::_paintEntryBlocks( const QVariant& blocks, PagedPNG* writer, const QRect& r, QPoint& cp ) {
    //
    // iterate over blocks
    //
    QSequentialIterable iterable = blocks.value<QSequentialIterable>();
...
void DiaryWriter::_paintEntryBlock( const QVariantMap& block, PagedPNG* writer, const QRect& r, QPoint& cp ) {
    QString type = block["type"].toString();
    QString content = block["content"].toString();
    QRect bounds( cp.x(), cp.y(), r.width() / 2, 0 );
    if ( type == "image" ) {
...
void DiaryWriter::_requestSpace( QRect& requested, PagedPNG* writer, const QRect& r, QPoint& cp) {
    if ( cp.y() + requested.height() > r.bottom() ) {
        //
        // adjust cp
        //
...
void DiaryWriter::_paintPageBackground(const QRect& bounds) {
